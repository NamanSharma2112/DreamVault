...
export default function WishlistPage() {
  ...
  const handleCreate = async (data: ItemCreateInput) => {
    try {
      await createItem.mutateAsync({ ...data, listType: "WISHLIST" });
  ...
  return (
    <div className="space-y-8">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="text-display-md text-ink">Wishlist</h1>
          <p className="mt-2 text-body text-ink-muted-80">
            Your aspirational dreams. Items here are waiting for the right moment.
          </p>
        </div>
      ...
      {isLoading ? (
        ...
        <div className="flex min-h-[400px] flex-col items-center justify-center rounded-[18px] border border-hairline bg-canvas-parchment p-8 text-center">
          <Heart className="mb-4 h-12 w-12 text-ink-muted-48" />
